  ...
  for (int t = 0; t < n_steps; t++) {
    ...
    for (size_t y = 1; y < NY-1; y++) {
      for (size_t x = 0; x < NX-7; x+=8) {
        __m256 from_current = _mm256_load_ps(&data[from][y][x]);
        const __m256 from_x_minus = _mm256_loadu_ps(&data[from][y][x-1]);
        const __m256 from_x_plus = _mm256_loadu_ps(&data[from][y][x+1]);
        const __m256 from_y_minus = _mm256_load_ps(&data[from][y-1][x]);
        const __m256 from_y_plus = _mm256_load_ps(&data[from][y+1][x]);
        ...
        _mm256_store_ps(&data[to][y][x], from_current);
      }
